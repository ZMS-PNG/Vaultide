...
describe('isContentEngineV3Enabled', () => {
  const flag = 'OPENMAIC_CONTENT_ENGINE_V3';
  let original: string | undefined;
  ...
  it('fails closed when the flag is missing, empty, or malformed', () => {
    delete process.env[flag];
    expect(isContentEngineV3Enabled()).toBe(false);
  ...
  it("enables V3 only for the standard explicit true values", () => {
    process.env[flag] = 'true';
    expect(isContentEngineV3Enabled()).toBe(true);
