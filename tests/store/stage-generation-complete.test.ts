...
describe('generationComplete', () => {
  it('keeps one materialized scene per outline order when a late generation result arrives', () => {
    useStageStore.getState().setStage(makeStage());
    useStageStore.getState().addScene(makeSlideScene('scene-first', 1));
    useStageStore.getState().addScene(makeSlideScene('scene-late-duplicate', 1));

    expect(useStageStore.getState().scenes.map((scene) => scene.id)).toEqual(['scene-first']);
  });
