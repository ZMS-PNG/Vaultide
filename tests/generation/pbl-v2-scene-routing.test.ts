...
describe('generateSceneContent — PBL v2 planner routing', () => {
  ...
  it('uses one bounded planner call and deterministic completion in the durable first pass', async () => {
    generatePBLV2ProjectSingleCallMock.mockRejectedValueOnce(new Error('invalid planner JSON'));
    projectV2ToLegacyProjectConfigMock.mockImplementation((project) => ({
    ...
    expect(generatePBLV2ProjectSingleCallMock).toHaveBeenCalledTimes(1);
    expect(generatePBLV2ProjectSingleCallMock.mock.calls[0][4]).toMatchObject({
      maxOutputTokens: 7_000,
      maxRetries: 0,
    ...
    expect(generatePBLV2ProjectMock).not.toHaveBeenCalled();
    expect(generatePBLContentMock).not.toHaveBeenCalled();
    expect(content?.projectV2?.title).toBe('CSV Data Analyzer');
    expect(content?.projectV2?.milestones).toHaveLength(3);
    expect(
      content?.projectV2?.milestones.every((milestone) => milestone.microtasks.length >= 1),
    ).toBe(true);
  });
});
