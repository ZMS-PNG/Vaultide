...
describe('task-engine outline route', () => {
  ...
  test('recovers a complete malformed JSON envelope after strict streaming extraction finds none', async () => {
    vi.resetModules();
    streamLLMMock.mockReset();
    resolveModelFromRequestMock.mockReset();
    ...
    const malformedEnvelope =
      '{languageDirective:"Teach in English.",courseTitle:"Recovered course",outlines:[{id:"scene_1",type:"slide",title:"Recovered scene",description:"Explain the recovered mechanism with evidence and a learner action.",keyPoints:["Mechanism","Evidence","Action"],order:1}]}';
    streamLLMMock.mockReturnValue({
      textStream: (async function* () {
        yield malformedEnvelope;
      })(),
    });
    ...
    const response = await POST({
      ...mockRequest({
        requirement: 'Recover a malformed outline response',
      }),
      json: async () => ({
        requirements: {
          requirement: 'Recover a malformed outline response',
    ...
    const done = events.find((event) => event.type === 'done');
    expect(done).toBeDefined();
    expect(done.courseTitle).toBe('Recovered course');
  ...
  test('converges a complete but uncited outline against frozen evidence without another model call', async () => {
    vi.resetModules();
    streamLLMMock.mockReset();
    resolveModelFromRequestMock.mockReset();
    ...
    const frozenSources = [
      '- [S1] Architecture specification: request boundaries, ownership, and state-transition rules.',
      '- [S2] Evaluation paper: measured results, limitations, and competing implementation evidence.',
      '- [S3] Operations guide: failure signals, recovery procedure, and acceptance checks.',
      '- [S4] Reference implementation: data flow, implementation choices, and verification artifacts.',
    ].join('\n');
    const researchContext = Array.from({ length: 16 }, () => frozenSources).join('\n');
    streamLLMMock.mockReturnValue({
      textStream: (async function* () {
        yield JSON.stringify({
          languageDirective: 'Teach in English.',
          courseTitle: 'Evidence convergence course',
          outlines: makeHighQualityOutlines(),
        });
      })(),
    });
    ...
    const { POST } = await import('@/lib/server/api-routes/generate/scene-outlines-stream/handler');
    const response = await POST({
      ...mockRequest({ requirement: 'Learn the selected project evidence' }),
      json: async () => ({
        requirements: {
          requirement: 'Learn the selected project evidence',
        },
        pdfText: '',
        pdfImages: [],
        imageMapping: {},
    ...
    const evidenceAssessment = assessOutlineEvidenceIntegrity(researchContext, done.outlines);
    expect(evidenceAssessment.passed, JSON.stringify(evidenceAssessment, null, 2)).toBe(true);
    expect(evidenceAssessment.metrics.citationCoverage).toBe(1);
    expect(evidenceAssessment.metrics.claimTraceability).toBe(1);
    expect(evidenceAssessment.metrics.sourceUtilization).toBe(1);
    expect(evidenceAssessment.metrics.finalTraceable).toBe(true);
  });

  test('repairs a missing keyPoints field instead of crashing', async () => {
    vi.resetModules();
    streamLLMMock.mockReset();
    resolveModelFromRequestMock.mockReset();
    ...
    const malformed = makeHighQualityOutlines().map((outline, index) =>
      index === 3
        ? ({
            ...outline,
            keyPoints: undefined,
          } as unknown as typeof outline)
        : outline,
    );
    streamLLMMock.mockReturnValue({
      textStream: (async function* () {
        yield JSON.stringify({
          languageDirective: 'Teach in English.',
          courseTitle: 'Malformed outline contract',
          outlines: malformed,
        });
      })(),
    });
    ...
    const { POST } = await import('@/lib/server/api-routes/generate/scene-outlines-stream/handler');
    const response = await POST({
      ...mockRequest({ requirement: 'Learn a project safely' }),
      json: async () => ({
        requirements: { requirement: 'Learn a project safely' },
        pdfText: '',
        pdfImages: [],
        imageMapping: {},
        researchContext: '',
    ...
    expect(error).toBeUndefined();
    expect(done).toBeDefined();
    expect(done.outlines[3].keyPoints.length).toBeGreaterThanOrEqual(3);
    expect(done.outlines[3].keyPoints.length).toBeLessThanOrEqual(5);
  });
});
