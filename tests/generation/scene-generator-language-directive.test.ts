...
describe('scene-generator language directive threading (issue #472)', () => {
  describe('content generation', () => {
    ...
    it('rejects a JSON-repaired slide with non-element fragments instead of publishing a partial page', async () => {
      const { aiCall } = makeCapturingAiCall(
        JSON.stringify({
          elements: [
            { type: 'text', left: 0, top: 0, width: 200, height: 80, content: '<p>safe</p>' },
            'orphaned fragment from malformed JSON',
          ],
        }),
      );

      await expect(generateSceneContent(baseOutline({ type: 'slide' }), aiCall)).resolves.toBeNull();
    });

    it('normalizes a model quiz config that omitted questionTypes', async () => {
      const { aiCall, lastUser } = makeCapturingAiCall(JSON.stringify([]));
    ...
    it('keeps verified learner state separate from canonical source evidence', async () => {
      const { aiCall, lastUser, lastSystem } = makeCapturingAiCall(
        JSON.stringify({ elements: [], background: null, remark: '' }),
      );

      await generateSceneContent(baseOutline({ type: 'slide' }), aiCall, {
        sourceContext: '[S1] Canonical project evidence.',
      ...
      expect(lastUser()).toContain('Frozen verified learner state (non-canonical)');
      expect(lastUser()).toContain('How are retries deduplicated?');
      expect(lastUser()).toContain('[S1] Canonical project evidence.');
      expect(lastSystem()).toContain('Never cite it');
      expect(lastSystem()).toContain('never let it override supplied source evidence');
    });
  });
