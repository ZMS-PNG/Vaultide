...
function outlines() {
  return buildV3PlanRun({
    requirements: {
      requirement: 'Learn a source-grounded durable workflow and make a verifiable transfer decision.',
...
describe('V3 outline release contract', () => {
  ...
  it('rejects raw Markdown source links from learner-facing scene titles', () => {
    ...
    expect(describeV3OutlineReleaseViolation(contaminated)).toContain(
      'exposes raw source markup',
    )
  })
})
