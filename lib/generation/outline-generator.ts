...
function removeInteractiveFields(outline: SceneOutline): SceneOutline {
  const normalized: SceneOutline = { ...outline };
...
function convertOutlineToSlide(outline: SceneOutline): SceneOutline {
  const normalized = removeInteractiveFields(outline);
  normalized.type = 'slide';
  delete normalized.quizConfig;
  return normalized;
}

function convertOutlineToRecallQuiz(outline: SceneOutline): SceneOutline {
  const normalized = removeInteractiveFields(outline);
  normalized.type = 'quiz';
  normalized.quizConfig = {
    questionCount: 3,
    difficulty: 'medium',
    questionTypes: ['single', 'text'],
  };
  normalized.description = `${outline.description} Use source-grounded active recall and application questions; do not test trivia.`;
  return normalized;
}
...
export function normalizeQualityFirstOutlines(outlines: SceneOutline[]): SceneOutline[] {
  ...
  if (normalized.length >= 6 && !normalized.some((outline) => outline.type === 'quiz')) {
    const recallPattern =
      /测验|检验|挑战|复习|回顾|决策|风险|验证|assessment|quiz|challenge|review|risk/i;
    const start = Math.max(1, Math.floor(normalized.length / 2));
    let candidate = -1;
    for (let index = normalized.length - 2; index >= start; index--) {
      if (
        normalized[index].type === 'interactive' &&
        recallPattern.test(`${normalized[index].title} ${normalized[index].description}`)
      ) {
        candidate = index;
        break;
      }
...
> {
  ...
  try {
    ...
    const enriched = normalizeQualityFirstOutlines(
      rawOutlines.map((outline, index) => ({
        ...outline,
    ...
    const quality = assessOutlineQuality(result);
    if (shouldEnforceCourseQuality() && !quality.passed) {
      return {
        success: false,
        error: `Outline quality contract failed: ${describeQualityIssues(quality, 8)}`,
      };
    }
