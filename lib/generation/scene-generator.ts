...
export interface SceneContentOptions {
  ...
  /**
   * Frozen source material used to design the course. A scene-specific evidence
   * window is selected from it and attached to every content prompt.
   */
  sourceContext?: string;
  /**
   * Server-verified learner state from a frozen knowledge snapshot. This is
   * used only to adapt instruction and target misconceptions; it must never be
   * treated as source evidence or cited as a factual authority.
   */
  learnerKnowledgeContext?: string;
  allowProceduralSkill?: boolean;
  /** Durable course build path: one bounded model call plus deterministic completion. */
  durableFirstPass?: boolean;
  /** Shared route deadline; required by generators that call the model directly. */
  abortSignal?: AbortSignal;
  /** Route-specific output budget, forwarded to direct model callers such as PBL. */
...
function learnerAwareCall(aiCall: AICallFn, learnerKnowledgeContext?: string): AICallFn {
  const learnerState = String(learnerKnowledgeContext ?? '')
    .trim()
    .slice(0, 24_000);
  if (!learnerState) return aiCall;
  return (systemPrompt, userPrompt, images) =>
    aiCall(
      `${systemPrompt}\n\n## Learner-state boundary\nThe learner-state block is non-canonical state, not factual source material. Use it only to avoid unnecessary repetition, correct named misconceptions, revisit unresolved questions, and choose the next appropriate difficulty. Never cite it, never let it override supplied source evidence, and never convert it into a factual claim without source support.`,
...
> {
  const {
    ...
    durableFirstPass = false,
    abortSignal,
    maxOutputTokens,
    editDirective,
    baselineContent,
  } = options;
  const adaptedAiCall = learnerAwareCall(aiCall, learnerKnowledgeContext);
  const adaptedRequirements =
    learnerKnowledgeContext?.trim() && userRequirements
      ? {
          ...userRequirements,
          userBio: [
            userRequirements?.userBio,
            'Use the frozen verified learner state supplied separately to target review and misconceptions. Do not treat it as factual source evidence.',
          ]
            .filter(Boolean)
            .join('\n'),
        }
  ...
  if (outline.type === 'interactive') {
    ...
    return generateWidgetContent(outline, adaptedAiCall, languageDirective, {
  ...
  switch (outline.type) {
    ...
    case 'quiz':
      return generateQuizContent(outline, adaptedAiCall, languageDirective, sourceContext);
    case 'pbl':
...
async function generateSlideContent(
  ...
  const blueprintPrompts = appendApprovedSceneBlueprint(prompts.system, prompts.user, outline);
  const evidencePrompts = appendUntrustedSourceEvidence(
    blueprintPrompts.system,
    blueprintPrompts.user,
    selectSceneSourceContext(sourceContext, outline),
  );
  const systemPrompt = evidencePrompts.system;
  let userPrompt = evidencePrompts.user;
  ...
  // jsonrepair can salvage a response with an unescaped quote, but that repair
  // may leave string fragments inside the `elements` array. Previously those
  // fragments were silently dropped below and a visually incomplete page could
  // still pass a text-heavy quality score. Treat the whole model payload as
  // unusable instead; the durable pipeline then publishes its complete,
  // outline-grounded first-pass scene with no learner-visible half-slide.
  const structurallyInvalid = generatedData.elements.some(
    (element) =>
      !element ||
      typeof element !== 'object' ||
      Array.isArray(element) ||
      !('type' in element) ||
      !['text', 'image', 'video', 'shape', 'chart', 'latex', 'line'].includes(
        String((element as { type?: unknown }).type ?? ''),
      ),
  );
  if (structurallyInvalid) {
    log.warn(`Discarding structurally repaired slide payload for: ${outline.title}`);
    return null;
  }
  ...
  if (fixedElements.length !== generatedData.elements.length) {
    log.warn(`Discarding partially normalized slide payload for: ${outline.title}`);
    return null;
  }
...
async function generateQuizContent(
  ...
  const quizConfig = {
    ...
    questionTypes:
      Array.isArray(outline.quizConfig?.questionTypes) &&
      outline.quizConfig.questionTypes.length > 0
        ? outline.quizConfig.questionTypes
        : ['single'],
  };
  ...
  const response = await aiCall(evidencePrompts.system, evidencePrompts.user);
  ...
  const normalizedQuestions: QuizQuestion[] = generatedQuestions.map((q) => {
  ...
  const questions = stabilizeGeneratedQuizAssessment(
    outline,
    normalizedQuestions,
    languageDirective,
  );
...
async function generatePBLSceneContent(
  ...
  if (v2Disabled && scenarioRoleplay && !runtimeOptions.durableFirstPass) {
    log.error(
  ...
  if (!v2Disabled) {
    ...
    if (runtimeOptions.durableFirstPass) {
      try {
        const projectV2 = await generatePBLV2ProjectSingleCall(
          plannerInput,
          languageModel,
          { onProgress },
          thinkingConfig,
        ...
        log.info(
          `PBL v2 generated (bounded first pass): ${projectV2.milestones.length} milestones, ${projectV2.roles.length} roles`,
        );
        return {
        ...
        log.warn(
          `Bounded PBL planner failed for "${outline.title}" (${message}); using deterministic complete project instead of another provider call.`,
        );
        const projectV2 = buildDeterministicPBLProject(
          plannerInput,
          sourceContext,
          scenarioRoleplay,
        );
...
function buildDeterministicPBLProject(
  ...
  const chinese =
    /Chinese|中文|zh-/iu.test(input.courseContext.languageDirective) ||
    /\p{Script=Han}/u.test(
      `${outline.title}${outline.description}${(outline.keyPoints ?? []).join('')}`,
    );
  const points = (outline.keyPoints ?? []).filter(Boolean);
  const skills = (config.targetSkills ?? []).filter(Boolean);
  ...
  const evidenceSuffix = evidenceLabels.length > 0 ? ` ${evidenceLabels.join(' ')}` : '';
  const topic = config.projectTopic || outline.title;
  const description = config.projectDescription || outline.description;

  project.title = topic;
  project.description = chinese
    ? `${description} 学习者将产出一个可以检查、运行或评审的真实成果，并以状态、测试、来源或验收记录证明结果，而不是只提交一段概述。${evidenceSuffix}`
    : `${description} The learner will produce a real artifact that can be inspected, run, or reviewed, and prove the result with state, tests, sources, or acceptance evidence rather than a prose-only summary.${evidenceSuffix}`;
  project.learningObjective =
    outline.teachingObjective ||
    (chinese
      ? `把${topic}的核心机制应用到一个新问题中，并能够解释决策、验证结果和失败边界。`
      : `Apply the core mechanism of ${topic} to a new problem and explain the decision, verification result, and failure boundary.`);
  project.gains = [...skills, ...points]
    .map((entry) => (chinese ? `能够运用并验证：${entry}` : `Apply and verify: ${entry}`))
    .filter(Boolean)
    .slice(0, 5);
  while ((project.gains?.length ?? 0) < 3) {
    project.gains!.push(
      chinese
        ? ['能够建立机制与结果的因果链', '能够用证据完成验收', '能够迁移到新的项目情境'][
            project.gains!.length
          ]
        : [
            'Build a causal chain from mechanism to result',
            'Complete acceptance with observable evidence',
            'Transfer the method to a new project context',
          ][project.gains!.length],
    );
  }

  const instructorId = newId('role');
  project.roles = [
    {
      id: instructorId,
      type: 'instructor',
      name: chinese ? '项目教练' : 'Project Coach',
      description: chinese
        ? '帮助你把理解转化为可验证成果，并在关键节点提供反馈。'
        : 'Helps turn understanding into a verifiable artifact and gives feedback at critical checkpoints.',
      systemPrompt: instructorProjectAnchor(project),
    },
  ];
  project.threads = [{ agentId: instructorId, messages: [] }];

  const exactPoints =
    points.length > 0 ? points : [outline.description, ...(skills.length > 0 ? skills : [topic])];
  const makeTask = (
    milestoneIndex: number,
    taskIndex: number,
    title: string,
    taskDescription: string,
    extra: Partial<PBLMicrotask> = {},
  ): PBLMicrotask => ({
    id: newId(`mt${milestoneIndex + 1}${taskIndex + 1}`),
    title,
    ...
    hints: chinese
      ? [
          '先写下当前判断和依据，再操作或修改成果。',
          '保留截图、测试结果、来源标签或状态记录作为证据。',
        ]
      : [
          'Write down the current judgment and evidence before changing the artifact.',
          'Keep a screenshot, test result, source label, or state record as evidence.',
        ],
    order: taskIndex,
    ...extra,
  });

  const ordinaryPhases = chinese
    ? [
        {
          title: '阶段一：建立机制与验收基线',
          briefing: `先把${topic}拆成输入、约束、状态转换和可观察结果，并明确每个核心要点如何影响最终成果。`,
          criteria: '提交一张机制链和一份验收清单；每条判断都能追溯到课程要点或来源证据。',
          debrief: '反馈重点：因果链是否完整、边界是否明确、证据是否足以支持下一阶段。',
        },
        {
          title: '阶段二：构建、测试与修正真实成果',
          briefing:
            '依据基线完成实际构建；每次关键决策都要留下理由、操作结果和失败现象，避免只描述“应该怎么做”。',
          criteria:
            '真实成果能够运行、检查或评审；关键路径至少经过一次正向验证和一次失败条件验证。',
          debrief: '反馈重点：结果是否由机制解释，修正是否针对第一个失效连接，而不是盲目反复尝试。',
        },
        {
          title: '阶段三：迁移挑战与最终验收',
          briefing: `把已验证的方法迁移到一个不同条件的新问题，比较哪些规则保持不变、哪些约束必须调整。${evidenceSuffix}`,
          criteria: '提交最终成果、验收证据、失败边界和迁移说明；结论必须能够被另一位学习者复现。',
          debrief: '反馈重点：能否独立解释机制、证明结果，并把方法用于新的项目、决策或问题。',
        },
      ]
    : [
        {
          title: 'Phase 1: Mechanism and acceptance baseline',
          briefing: `Decompose ${topic} into inputs, constraints, state transitions, and observable results, then connect every core learning point to the artifact.`,
          criteria:
            'Submit a mechanism map and acceptance checklist; every judgment is traceable to an approved learning point or source.',
          debrief:
            'Feedback focuses on causal completeness, explicit boundaries, and evidence strong enough to start the build.',
        },
        {
          title: 'Phase 2: Build, test, and correct the real artifact',
          briefing:
            'Build the actual artifact from the baseline. Record the reason, operation, result, and failure observation for each important decision instead of describing what should happen.',
          criteria:
            'The artifact can be run, inspected, or reviewed, with one successful-path test and one failure-condition test.',
          debrief:
            'Feedback focuses on whether the mechanism explains the result and whether correction targets the first broken connection.',
        },
        {
          title: 'Phase 3: Transfer challenge and final acceptance',
          briefing: `Transfer the verified method to a new problem with different conditions. Compare what remains invariant and what must change.${evidenceSuffix}`,
          criteria:
            'Submit the final artifact, acceptance evidence, failure boundary, and transfer note so another learner can reproduce the result.',
          debrief:
            'Feedback focuses on independent explanation, verifiable results, and transfer to a new project, decision, or problem.',
        },
      ];

  if (scenarioRoleplay) {
    const scenarioBrief = config.scenarioBrief || description;
    project.scenario = {
      setting: scenarioBrief,
      goal: project.learningObjective,
      learnerRole: chinese
        ? `你是${topic}情境中的决策者。`
        : `You are the decision maker in ${topic}.`,
      rules: chinese
        ? '根据对方反应调整表达；每一步都要体现课程要点，并在结尾说明依据。'
        : 'Adapt to the character response, use the course mechanisms, and explain the evidence at the end.',
      characters: [
        {
          id: newId('char'),
          name: chinese ? '情境伙伴' : 'Scenario Partner',
          persona: chinese
            ? '会根据学习者的表达提供真实、克制且有边界的反馈。'
            : 'Responds realistically and gives restrained, bounded feedback to the learner.',
          situation: scenarioBrief,
          boundaries: chinese
            ? '不替学习者给出完整答案，不越过安全与事实边界。'
            : 'Does not solve the task for the learner or cross safety and factual boundaries.',
          openingLine: chinese
            ? '我们开始吧，你准备如何处理当前情境？'
            : 'Let us begin. How will you handle this situation?',
        },
      ],
      sceneVisual: {
        caption: topic,
    ...
    const stages: Array<'prep' | 'roleplay' | 'wrapup'> = ['prep', 'roleplay', 'wrapup'];
    project.milestones = ordinaryPhases.map(
      (phase, index): PBLMilestone => ({
        id: newId(`ms${index + 1}`),
        title: phase.title,
        description: `${phase.briefing} ${exactPoints.join('；')}`,
        status: index === 0 ? 'active' : 'locked',
        order: index,
        scenarioStage: stages[index],
        ...
        microtasks: [
          makeTask(
            ...
            index === 0
              ? chinese
                ? '确认角色、目标与判断依据'
                : 'Confirm role, objective, and evidence'
              : index === 1
                ? chinese
                  ? '完成关键对话并观察反馈'
                  : 'Complete the pivotal exchange and observe feedback'
                : chinese
                  ? '复盘、迁移并提交可验证结论'
                  : 'Debrief, transfer, and submit a verifiable conclusion',
            `${phase.briefing} ${exactPoints[index % exactPoints.length]}`,
            index === 1
              ? {
                  successWhen: chinese
                    ? '学习者完成一个明确行动或表达，并根据角色反馈解释下一步决策。'
                    : 'The learner takes a concrete action or says a clear response, then explains the next decision from the character feedback.',
                  completionCriteria: phase.criteria,
                  skillFocus: exactPoints[index % exactPoints.length],
                  learnerBrief: phase.briefing,
                  characterObjective: chinese
                    ? '检验学习者是否真正理解机制并能据反馈调整。'
                    : 'Test whether the learner understands the mechanism and can adapt to feedback.',
                }
              : {},
          ),
        ],
      ...
      (phase, index): PBLMilestone => ({
        ...
        microtasks: [
          makeTask(
            index,
            0,
            chinese ? '产出本阶段成果' : 'Produce the phase artifact',
            `${phase.briefing} ${exactPoints[index % exactPoints.length]}`,
          ),
          makeTask(
            index,
            1,
            chinese ? '验证结果并记录反馈' : 'Verify the result and record feedback',
            chinese
              ? `${phase.criteria} 记录成功结果、一个失败条件、修正动作与重新验证结果。`
              : `${phase.criteria} Record the successful result, one failure condition, the correction, and the verification after correction.`,
          ),
        ],
      }),
    );
...
export async function generateWidgetContent(
  ...
  aiCall: AICallFn,
  languageDirective?: string,
  options: { allowProceduralSkill?: boolean; sourceContext?: string } = {},
  ...
  if (widgetType === 'diagram') {
    const diagram = buildDeterministicDiagram(outline, languageDirective);
    const processedHtml = postProcessInteractiveHtml(diagram.html);
    log.info(
      `Generated deterministic diagram for "${outline.title}" (${diagram.widgetConfig.nodes.length} nodes; model call skipped)`,
    );
    return {
      html: processedHtml,
      widgetType,
  ...
  const evidencePrompts = appendUntrustedSourceEvidence(
    blueprintPrompts.system,
    blueprintPrompts.user,
    selectSceneSourceContext(options.sourceContext, outline),
  );
  const response = await aiCall(evidencePrompts.system, evidencePrompts.user);
