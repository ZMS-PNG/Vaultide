...
function usageDir(baseDir?: string): string {
  ...
  // Vercel's deployment bundle lives under read-only /var/task. Usage logging
  // is best-effort, so use the writable ephemeral area instead of emitting an
  // ENOENT warning after every successful generation.
  if (process.env.VERCEL) return path.join('/tmp', 'openmaic', 'usage');
