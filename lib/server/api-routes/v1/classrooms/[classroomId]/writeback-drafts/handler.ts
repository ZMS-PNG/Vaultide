...
export async function POST(
  ...
  try {
    ...
    const progress = parseProgressSnapshot(await readLearningJson(request));
    if (!progress)
      return learningError(context, 'invalid_request', 400, 'Invalid progress snapshot.');
    const draft = await getLearningProgressService().createWritebackDraft(classroomId, progress);
