...
export interface UserRequirements {
  ...
  /**
   * Whether external evidence is disabled, supplemental to a canonical private
   * source, or mandatory before generation may continue.
   */
  externalEvidenceMode?: import('@/lib/generation/external-evidence-policy').ExternalEvidenceMode;
  externalEvidenceStatus?: import('@/lib/generation/external-evidence-policy').ExternalEvidenceStatus;
  ...
  /** Goal, source boundary, learner starting point, and observable success criteria. */
  learningProject?: import('@/lib/learning/domain/learning-project-plan').LearningProjectBrief;
  /**
   * Immutable V3 learning intent. It is created before source planning and is
   * persisted with the planning run so a retry cannot silently change the
   * learner's goal, source boundary, or required artifact.
   */
  learningContract?: import('@/lib/learning/domain/v3/learning-contract').LearningContract;
}
 ...
 */
export type InstructionalActivityKind =
  | 'orientation'
  | 'diagnostic'
  | 'foundation'
  | 'mechanism'
...
/**
 * A deterministic, learner-visible job for one scene. This is intentionally
 * separate from the scene renderer: model output may elaborate the scene, but
 * it may not erase the planned evidence, learner action, or acceptance result.
 */
export interface InstructionalActivityContract {
  schemaVersion: 3;
  slotId: string;
  ...
  /**
   * Present only for the final V3 transfer activity. Keeping this contract on
   * the outline makes the required learner deliverable durable across planner,
   * renderer, retry, and Obsidian writeback boundaries.
   */
  artifact?: {
    artifactType: string;
...
export interface SceneOutline {
  ...
  /**
   * A compact, verbatim claim from the frozen source that this scene teaches.
   * It is planner-only evidence-routing data: V3 verifies it against the
   * source before using it in learner-visible content.
   */
  ...
  activity?: InstructionalActivityContract;
  /**
   * Durable-worker-only repair guidance. API handlers consume this field as
   * an internal prompt suffix and remove it before creating learner-visible
   * content, actions, remarks, or classroom metadata.
   */
  generationRepairDirective?: string;
