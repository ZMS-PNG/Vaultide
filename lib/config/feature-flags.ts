...
/**
 * V3 keeps planning, evidence and quality contracts explicit. It is a
 * server-only, opt-in capability: a missing, empty, or malformed value must
 * keep the legacy path in control. That makes a deployment/configuration
 * mistake safe and prevents an unverified rollout from silently becoming the
 * production default.
 *
 * Operators enable this only for the approved canary by setting
 * `OPENMAIC_CONTENT_ENGINE_V3=true` (or `1`). An explicit `false` is the
 * immediate rollback switch; it never deletes a persisted plan or course.
 */
export function isContentEngineV3Enabled(): boolean {
  return readBoolean(process.env.OPENMAIC_CONTENT_ENGINE_V3);
}
