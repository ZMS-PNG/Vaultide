...
export interface StageRecord {
  ...
  whiteboard?: Whiteboard[]; // Classroom whiteboard pages
  learningContext?: StageLearningContext; // Source provenance and learning goal
}
...
export interface StageOutlinesRecord {
  ...
  // Per-classroom resume data. This replaces the former shared sessionStorage
  // checkpoint, which could be overwritten whenever another course started.
  generationParams?: PersistedGenerationParams;
  generationStatus?: PersistedGenerationStatus;
  generationFailedOutlineIds?: string[];
...
export interface PersistedGenerationParams {
  ...
  userProfile?: string;
  languageDirective?: string;
  /** Frozen evidence carried across refresh so later pages stay source-grounded. */
  sourceContext?: string;
  requirements?: import('@/lib/types/generation').UserRequirements;
}

export type PersistedGenerationStatus = 'pending' | 'generating' | 'paused' | 'completed';
