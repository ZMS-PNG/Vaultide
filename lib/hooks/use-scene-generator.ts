...
const DEFAULT_SCENE_REQUEST_TIMEOUT_MS = 290_000;
const INTERACTIVE_SCENE_REQUEST_TIMEOUT_MS = 290_000;
...
function createRequestTimeoutError(timeoutMs: number): Error & {
  ...
  const error = new Error(
    `Scene generation timed out after ${Math.round(timeoutMs / 1000)} seconds. Please retry this scene.`,
...
async function fetchWithRequestTimeout(
  ...
  try {
    ...
    if (timedOut) throw createRequestTimeoutError(timeoutMs);
    throw error;
  } finally {
...
export async function fetchSceneContent(
  ...
  const { requestTimeoutMs = DEFAULT_SCENE_REQUEST_TIMEOUT_MS, ...generationRetryOptions } =
    retryOptions ?? {};
      ...
      async () => {
        const response = await fetchWithRequestTimeout(
          ...
          params.outline.type === 'interactive'
            ? Math.min(requestTimeoutMs, INTERACTIVE_SCENE_REQUEST_TIMEOUT_MS)
            : requestTimeoutMs,
        );
...
export function useSceneGenerator(options: UseSceneGeneratorOptions = {}) {
  ...
  const generateRemaining = useCallback(
    async (params: GenerationParams) => {
      ...
      if (pending.length === 0) {
        const courseQuality = assessCourseQuality(outlines, scenes);
        if (!courseQuality.passed) {
          store.getState().setGenerationStatus('paused');
          const qualityMessage = `Course quality review failed: ${describeQualityIssues(courseQuality, 8)}`;
          const failedOrders = new Set(
            courseQuality.issues
              .map((entry) => entry.sceneOrder)
              .filter((order): order is number => order !== undefined),
          );
          for (const failedOutline of outlines.filter((outline) =>
            failedOrders.has(outline.order),
          )) {
            store.getState().addFailedOutline(failedOutline);
            optionsRef.current.onSceneFailed?.(failedOutline, qualityMessage);
          }
        } else {
      ...
      try {
        ...
        for (const outline of pending) {
          ...
          if (!contentResult.success || !contentResult.content) {
            ...
            optionsRef.current.onSceneFailed?.(
              outline,
              contentResult.error || 'Content generation failed',
            );
            // A malformed or timed-out chapter should not turn a ten-page
            // course into a one-page course. Keep the failed outline visible
            // for an explicit retry and continue materializing later chapters.
            hadSceneFailure = true;
            continue;
          }
          ...
          const actionsResult = await fetchSceneActions(
            {
              outline: contentResult.effectiveOutline || contentOutline,
              allOutlines: outlines,
          ...
          if (actionsResult.success && actionsResult.scene) {
            ...
            if (
              ...
              if (!ttsResult.success) {
                ...
                // Audio is an enhancement, not the source of truth for the
                // lesson. Keep the quality-approved page and its speech text;
                // actions whose provider clips failed have no audioId and can
                // fall back to browser-native speech instead of deleting the page.
                log.warn(
                  `Scene "${outline.title}" kept with ${ttsResult.failedCount} missing audio clip(s): ${ttsResult.error ?? 'TTS generation failed'}`,
                );
              }
            }
            ...
            // Flush each completed page before starting the next one. This
            // makes refresh, route changes, and transient server errors resume
            // from the last durable page instead of from the first page.
        ...
        if (!abortRef.current && !interrupted) {
          if (hadSceneFailure) {
            // Keep failed outlines available for targeted retry after all other
            // chapters have been generated.
            store.getState().setGenerationStatus('paused');
          } else {
            const finalState = store.getState();
            const courseQuality = assessCourseQuality(
              finalState.outlines,
              finalState.scenes,
            );
            if (!courseQuality.passed) {
              ...
              for (const failedOutline of finalState.outlines.filter((candidate) =>
                failedOrders.has(candidate.order),
              )) {
                store.getState().addFailedOutline(failedOutline);
                optionsRef.current.onSceneFailed?.(failedOutline, qualityMessage);
              }
  ...
  // Keep ref in sync outside render so retrySingleOutline can resume the queue
  // without violating React's ref purity contract.
  useEffect(() => {
    ...
    async (outlineId: string) => {
      ...
      try {
        ...
        if (
          ...
          if (!ttsResult.success) {
            log.warn(
              `Retried scene "${outline.title}" kept with ${ttsResult.failedCount} missing audio clip(s): ${ttsResult.error ?? 'TTS generation failed'}`,
            );
          }
        }
