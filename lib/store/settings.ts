...
const getDefaultAudioConfig = () => ({
  ...
  ttsProvidersConfig: {
    ...
    // Browser-native speech is the zero-configuration classroom fallback. It
    // keeps private learning deployments audible without another API key.
  ...
  persist(
    (set, get) => {
      ...
      return {
        ...
        // Classroom narration works without another API key by using local
        // browser speech synthesis. A configured server provider can replace it.
        ttsEnabled: true,
        asrEnabled: true,
      ...
      migrate: (persistedState: unknown, version: number) => {
        ...
        if (!state.ttsProvidersConfig || !state.asrProvidersConfig) {
          ...
          for (const [key, value] of Object.entries(defaultAudioConfig)) {
            if (stateRecord[key] === undefined) stateRecord[key] = value;
          }
        }
        ...
        // v4 -> v5: enable the zero-configuration classroom narration
        // fallback. v4 selected browser-native while leaving both switches
        // off, so playback silently ran reading timers instead of speaking.
        if (version < 5) {
          const browserConfig = state.ttsProvidersConfig?.['browser-native-tts'];
          if (browserConfig) browserConfig.enabled = true;
          if (!state.ttsProviderId || state.ttsProviderId === 'browser-native-tts') {
