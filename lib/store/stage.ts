...
const useStageStoreBase = create<StageState>()((set, get) => ({
  ...
  addScene: (scene) => {
    ...
    const nextScene = migrateScene(scene);
    const existing = get().scenes.find(
      (candidate) => candidate.id === nextScene.id || candidate.order === nextScene.order,
    );
    // Scene content may finish after a retry, route transition, or a second
    // client resumes the same classroom. A course outline owns exactly one
    // materialized scene per order, so accepting a late duplicate would make
    // progress and writeback snapshots disagree with the course plan.
    if (existing) {
      log.warn(
        `Ignoring duplicate scene "${nextScene.title}" (incoming: ${nextScene.id}/${nextScene.order}, existing: ${existing.id}/${existing.order})`,
      );
      return;
    }
    const scenes = [...get().scenes, nextScene].sort((a, b) => a.order - b.order);
  ...
  saveToStorage: async () => {
    ...
    try {
      ...
      // Local persistence is authoritative for offline continuity; the server
      // snapshot makes the same classroom URL recoverable across browsers and
      // devices. Keep this best-effort so a temporary network failure never
      // destroys local work. A later document save retries the latest snapshot.
      if (typeof window !== 'undefined' && persistedScenes.length > 0) {
        void import('@/lib/classroom/sync-classroom')
          .then(({ queueClassroomSnapshotSync }) =>
            queueClassroomSnapshotSync(stage, persistedScenes, fetch, {
              outlines,
              generationComplete,
              generationStatus:
                generationComplete || generationStatus === 'completed'
                  ? 'completed'
                  : generationStatus === 'generating'
                    ? 'generating'
                    : generationStatus === 'paused' || generationStatus === 'error'
                      ? 'paused'
                      : 'pending',
              failedOutlineIds: failedOutlines.map((outline) => outline.id),
            }),
          )
          .catch((error) => log.warn('Failed to sync classroom snapshot:', error));
      }
