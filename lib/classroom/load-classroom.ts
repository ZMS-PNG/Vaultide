...
import { markClassroomSnapshotPersisted } from '@/lib/classroom/sync-classroom';
...
export interface RunClassroomLoadArgs<TMediaTasks = unknown> {
  ...
  getCurrentOutlines: () => readonly SceneOutline[];
...
export async function runClassroomLoad<TMediaTasks = unknown>({
  ...
  try {
    ...
    const needsServerClassroom = !localStage;
    const needsServerGenerationPlan = Boolean(localStage && getCurrentOutlines().length === 0);

    if (needsServerClassroom || needsServerGenerationPlan) {
      log.info(
        needsServerClassroom
          ? 'No IndexedDB data, trying server-side storage for:'
          : 'Local classroom has no generation plan, checking server snapshot for:',
        classroomId,
      );
      ...
      if (classroom) {
        const { stage, scenes, generation } = classroom;
        if (needsServerClassroom) {
        ...
        if (generation?.outlines.length) {
          const currentScenes = useStageStore.getState().scenes;
          useStageStore.setState({
            outlines: generation.outlines,
            generationComplete: generation.generationComplete,
            generationStatus:
              generation.generationStatus === 'pending' ? 'idle' : generation.generationStatus,
            generatingOutlines: generation.generationComplete
              ? []
              : generation.outlines.filter(
                  (outline) => !currentScenes.some((scene) => scene.order === outline.order),
                ),
            failedOutlines: generation.outlines.filter((outline) =>
              generation.failedOutlineIds.includes(outline.id),
            ),
          });
          const { updateStageOutlinesRecord } =
            await import('@/lib/generation/classroom-generation-checkpoint');
          await updateStageOutlinesRecord(stage.id, {
            outlines: generation.outlines,
            generationComplete: generation.generationComplete,
            generationStatus: generation.generationStatus,
            generationFailedOutlineIds: generation.failedOutlineIds,
          });
          await useStageStore.getState().saveToStorage();
        }
      } else if (needsServerClassroom) {
        setError('课堂不存在或尚未同步到服务器（Classroom not found）');
        return;
      }
    }
...
export function applyClassroomStageAndScenes(
  ...
  // The server is already authoritative for this snapshot. Mark it before
  // writing the offline IndexedDB copy so that local persistence does not
  // immediately POST the identical classroom back and bump its revision.
  markClassroomSnapshotPersisted(stage, nextScenes);
