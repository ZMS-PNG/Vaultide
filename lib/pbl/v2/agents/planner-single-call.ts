...
export interface PBLV2SingleCallRuntimeOptions {
  abortSignal?: AbortSignal;
  maxOutputTokens?: number;
  maxRetries?: number;
  /**
   * Manual/editor generation can ask the planner to correct malformed JSON
   * once. Durable course generation disables this because its caller provides
   * a deterministic complete project when the one bounded call fails.
   */
  allowTargetedRetry?: boolean;
}
...
export async function generatePBLV2ProjectSingleCall(
  ...
  runtimeOptions: PBLV2SingleCallRuntimeOptions = {},
    ...
    const result = await callLLM(
      {
        ...
        maxRetries: runtimeOptions.maxRetries ?? 0,
  ...
  if (gaps.length > 0 && runtimeOptions.allowTargetedRetry !== false) {
    log.warn(
