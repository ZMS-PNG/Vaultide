...
export function Roundtable({
  ...
  const [presentationPortalContainer, setPresentationPortalContainer] = useState<HTMLElement | null>(
    null,
  );
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      setPresentationPortalContainer(fullscreenContainerRef?.current ?? null);
    });
    return () => window.cancelAnimationFrame(frame);
  }, [fullscreenContainerRef, isPresenting]);

  // End flash effect (Issue 3)
  useEffect(() => {
    if (showEndFlash) {
      const showTimer = window.setTimeout(() => setEndFlashVisible(true), 0);
      const hideTimer = window.setTimeout(() => setEndFlashVisible(false), 1800);
      return () => {
        window.clearTimeout(showTimer);
        window.clearTimeout(hideTimer);
      };
    }
    const hideTimer = window.setTimeout(() => setEndFlashVisible(false), 0);
                ...
                <AnimatePresence>
                  {discussionRequest && (
                    <ProactiveCard
                      ...
                      portalContainer={presentationPortalContainer}
                      align="left"
                      agentName={
  ...
  return (
    <div
      className={cn(
        'h-[clamp(148px,17vh,176px)] w-full flex flex-col relative z-10 transition-all duration-300',
