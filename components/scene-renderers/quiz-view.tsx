...
export function QuizView({ questions, sceneId, stageId }: QuizViewProps) {
  ...
  const viewLifetime = useMemo(() => createQuizViewLifetime(), []);
  const runtimeWriter = useMemo(() => createQuizAttemptWriter({
    onError: (error) => log.warn('Failed to persist quiz runtime:', error),
  }), []);
  const learningAttemptIdRef = useRef<string | null>(null);
  ...
  const handleSubmit = useCallback(async () => {
    if (!attemptId) return;
    learningAttemptIdRef.current = crypto.randomUUID();
    setPhase('submitting');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (classroomId && learningAttemptId) {
        ...
        void recordClassroomLearningEvents(classroomId, [
          {
            ...
            clientEventId: `quiz:${learningAttemptId}`,
          ...
          },
          {
            ...
            clientEventId: `quiz-completed:${attemptId}`,
            occurredAt: new Date().toISOString(),
            payload: {
              sceneId,
              completionKind: 'quiz-submitted',
            },
          },
        ]).catch((eventError) => {
          log.warn('[quiz-view] Learning progress sync failed:', eventError);
        });
      }
    })();
