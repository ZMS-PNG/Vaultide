...
export function AgentRevealModal({ agents, open, onClose, onAllRevealed }: AgentRevealModalProps) {
  ...
  useEffect(() => {
    if (!open) {
      ...
      // Defer the visual reset until after React has committed the closed
      // state. This prevents a synchronous effect update from cascading into
      // the exit animation while preserving a clean next reveal.
