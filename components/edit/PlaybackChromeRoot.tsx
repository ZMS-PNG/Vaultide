...
export const PlaybackChromeRoot = forwardRef<PlaybackChromeRootHandle, PlaybackChromeRootProps>(
  function PlaybackChromeRoot({ onRetryOutline, canEnterProMode, onEnterProMode }, ref) {
    ...
    // This flag describes publication readiness only. It must never be used as
    // evidence that the learner has completed or mastered the course.
    const isCoursePublished =
      generationComplete ||
      (outlines.length > 0 && scenes.length === outlines.length && generatingOutlines.length === 0);
    const canAdvanceToPendingSlot = hasNextPending || isCoursePublished;
    ...
    const handleNextScene = useCallback(() => {
      ...
      if (currentIndex < scenes.length - 1) {
        ...
        // On the last real scene, advance to generation status or the publication checkpoint.
    ...
    // While the classroom is still materializing, show the planned course
    // length instead of only the scenes that have arrived so learners do not
    // see the denominator grow from 2 to 5 during an otherwise normal lesson.
    // Once a published course is edited, its rendered scene count remains the
    // source of truth because outlines can intentionally outlive deleted slides.
    const totalScenesCount = generationComplete
      ? scenes.length
      : Math.max(outlines.length, scenes.length + (canAdvanceToPendingSlot ? 1 : 0));
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0 relative">
          ...
          {!isPresenting && (
            <Header
              currentSceneTitle={
                currentScene?.title ||
                (isCoursePublished && isPendingScene ? stage?.name || '课程内容已准备' : '')
              }
              mode={mode}
          ...
          <div className="overflow-hidden relative flex-1 min-h-0 isolate" suppressHydrationWarning>
            <CanvasArea
              ...
              isCourseComplete={isCoursePublished}
              isGenerationFailed={
