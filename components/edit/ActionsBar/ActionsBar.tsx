...
export function ActionsBar({ sceneId }: { sceneId: string }) {
  ...
  const items = actions.reduce<
    Array<{ action: (typeof actions)[number]; index: number; key: string; speechIndex: number }>
  >((result, action, index) => {
    const speechIndex = result.at(-1)?.speechIndex ?? 0;
    result.push({
      action,
      index,
      key: (action.id ?? `a-${index}`) as string,
      speechIndex: action.type === 'speech' ? speechIndex + 1 : speechIndex,
    });
    return result;
  }, []);
