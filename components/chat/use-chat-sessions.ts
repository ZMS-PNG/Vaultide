...
export function useChatSessions(options: UseChatSessionsOptions = {}) {
  ...
  const sendMessage = useCallback(
    async (content: string): Promise<void> => {
      ...
      if (currentState.stage?.id && (sessionType === 'qa' || sessionType === 'discussion')) {
        void recordClassroomLearningEvents(currentState.stage.id, [
          {
            eventType: 'discussionParticipated',
            clientEventId: createBrowserLearningEventId('discussion-message'),
            occurredAt: new Date(now).toISOString(),
            payload: {
              ...(currentState.currentSceneId ? { sceneId: currentState.currentSceneId } : {}),
              sessionId: sessionId!,
              sessionType,
              messageLength: content.trim().length,
            },
          },
        ]).catch((error) => log.warn('Unable to record discussion participation:', error));
      }
